...
public class Graphics
{
	...
	private gr.Scene _scene;
	private readonly gr.ImGuiRenderable _igRenderable;
	private readonly gr.SceneContext _sc = new();
	private bool _windowResized;
	private gr.RenderOrderKeyComparer _renderOrderKeyComparer = new();
	...
	private static gr.FrameTimeAverager _fta = new(0.666);
	...
	private gr.Sdl2ControllerTracker _controllerTracker;
	private bool _colorSrgb = true;
	private gr.obj.FullScreenQuad _fsq;
	...
	public Graphics()
	{
		...
		gr.Sdl2ControllerTracker.CreateDefault( out _controllerTracker );

		_scene = new gr.Scene( _gd, _window, _controllerTracker );

		_sc.SetCurrentScene( _scene );

		_igRenderable = new gr.ImGuiRenderable( _window.Width, _window.Height );
		_resizeHandled += ( w, h ) => _igRenderable.WindowResized( w, h );
		_scene.AddRenderable( _igRenderable );
		_scene.AddUpdateable( _igRenderable );

		gr.obj.Skybox skybox = gr.obj.Skybox.LoadDefaultSkybox();
		...
		gr.obj.ShadowmapDrawer texDrawIndexeder = new gr.obj.ShadowmapDrawer(() => _window, () => _sc.NearShadowMapView);
		...
		gr.obj.ShadowmapDrawer texDrawIndexeder2 = new gr.obj.ShadowmapDrawer(() => _window, () => _sc.MidShadowMapView);
		...
		gr.obj.ShadowmapDrawer texDrawIndexeder3 = new gr.obj.ShadowmapDrawer(() => _window, () => _sc.FarShadowMapView);
		...
		gr.obj.ShadowmapDrawer reflectionTexDrawer = new gr.obj.ShadowmapDrawer(() => _window, () => _sc.ReflectionColorView);
		...
		gr.obj.ScreenDuplicator duplicator = new gr.obj.ScreenDuplicator();
		_scene.AddRenderable( duplicator );

		_fsq = new gr.obj.FullScreenQuad();
	...
	private void AddSponzaAtriumObjects()
	{
		...
		using( FileStream objStream = File.OpenRead( gr.AssetHelper.GetPath( "Models/SponzaAtrium/sponza.obj" ) ) )
		{
			ObjFile atriumFile = parser.Parse(objStream);
			MtlFile atriumMtls;
			using( FileStream mtlStream = File.OpenRead( gr.AssetHelper.GetPath( "Models/SponzaAtrium/sponza.mtl" ) ) )
			{
			...
			foreach( ObjFile.MeshGroup group in atriumFile.MeshGroups )
			{
				...
				gr.obj.MaterialPropsAndBuffer materialProps = gr.obj.CommonMaterials.Brick;
				if( materialDef.DiffuseTexture != null )
				{
					string texturePath = gr.AssetHelper.GetPath("Models/SponzaAtrium/" + materialDef.DiffuseTexture);
					overrideTextureData = LoadTexture( texturePath, true );
				}
				if( materialDef.AlphaMap != null )
				{
					string texturePath = gr.AssetHelper.GetPath("Models/SponzaAtrium/" + materialDef.AlphaMap);
					alphaTexture = LoadTexture( texturePath, false );
				}
				if( materialDef.Name.Contains( "vase" ) )
				{
					materialProps = gr.obj.CommonMaterials.Vase;
				}
				if( group.Name == "sponza_117" )
				{
					gr.obj.MirrorMesh.Plane = Plane.CreateFromVertices(
					...
					materialProps = gr.obj.CommonMaterials.Reflective;
				}
	...
	private void AddTexturedMesh(
			...
			gr.obj.MaterialPropsAndBuffer materialProps,
			Vector3 position,
			Quaternion rotation,
			Vector3 scale,
			string name )
	{
		gr.obj.TexturedMesh mesh = new gr.obj.TexturedMesh(name, meshData, texData, alphaTexData, materialProps ?? gr.obj.CommonMaterials.Brick);
	...
	public void Run()
	{
		...
		{
			previousFrameTicks = RunPass( previousFrameTicks, sw );
	...
	public long RunPass( long previousFrameTicks, Stopwatch sw )
	{
		...
		gr.InputTracker.UpdateFrameInput( snapshot, _window );
		Update( (float)deltaSeconds );


		if( !_window.Exists )
		{
			return previousFrameTicks;
		}
	...
	private void Update( float deltaSeconds )
	{
		...
		if( ImGui.BeginMainMenuBar() )
		{
			if( ImGui.BeginMenu( "Settings" ) )
			{
				if( ImGui.BeginMenu( "Graphics Backend" ) )
				{
					...
					if( ImGui.MenuItem( "Vulkan", string.Empty, _gd.BackendType == GraphicsBackend.Vulkan, Veldrid.GraphicsDevice.IsBackendSupported( GraphicsBackend.Vulkan ) ) )
					{
						ChangeBackend( GraphicsBackend.Vulkan );
					}
					if( ImGui.MenuItem( "OpenGL", string.Empty, _gd.BackendType == GraphicsBackend.OpenGL, Veldrid.GraphicsDevice.IsBackendSupported( GraphicsBackend.OpenGL ) ) )
					{
						ChangeBackend( GraphicsBackend.OpenGL );
					}
					if( ImGui.MenuItem( "OpenGL ES", string.Empty, _gd.BackendType == GraphicsBackend.OpenGLES, Veldrid.GraphicsDevice.IsBackendSupported( GraphicsBackend.OpenGLES ) ) )
					{
						ChangeBackend( GraphicsBackend.OpenGLES );
					}
					if( ImGui.MenuItem( "Direct3D 11", string.Empty, _gd.BackendType == GraphicsBackend.Direct3D11, Veldrid.GraphicsDevice.IsBackendSupported( GraphicsBackend.Direct3D11 ) ) )
					{
						ChangeBackend( GraphicsBackend.Direct3D11 );
					}
					if( ImGui.MenuItem( "Metal", string.Empty, _gd.BackendType == GraphicsBackend.Metal, Veldrid.GraphicsDevice.IsBackendSupported( GraphicsBackend.Metal ) ) )
					{
			...
			if( ImGui.BeginMenu( "Materials" ) )
			{
				...
				{
					DrawIndexedMaterialMenu( gr.obj.CommonMaterials.Brick );
					ImGui.EndMenu();
				}
				if( ImGui.BeginMenu( "Vase" ) )
				{
					DrawIndexedMaterialMenu( gr.obj.CommonMaterials.Vase );
					ImGui.EndMenu();
				}
				if( ImGui.BeginMenu( "Reflective" ) )
				{
					DrawIndexedMaterialMenu( gr.obj.CommonMaterials.Reflective );
					ImGui.EndMenu();
				}
		...
		if( gr.InputTracker.GetKeyDown( Key.F11 ) )
		{
			ToggleFullscreenState();
		}

		if( gr.InputTracker.GetKeyDown( Key.Keypad6 ) )
		{
			_window.X += 10;
		}
		if( gr.InputTracker.GetKeyDown( Key.Keypad4 ) )
		{
			_window.X -= 10;
		}
		if( gr.InputTracker.GetKeyDown( Key.Keypad8 ) )
		{
			_window.Y += 10;
		}
		if( gr.InputTracker.GetKeyDown( Key.Keypad2 ) )
		{
			_window.Y -= 10;
		}
	...
	private void DrawIndexedMaterialMenu( gr.obj.MaterialPropsAndBuffer propsAndBuffer )
	{
		gr.MaterialProperties props = propsAndBuffer.Properties;
	...
	private void Draw()
	{
		...
		gr.obj.CommonMaterials.FlushAll( _frameCommands );
	...
	private void ChangeBackend( GraphicsBackend backend, bool forceRecreateWindow )
	{
		...
		Veldrid.GraphicsDeviceOptions gdOptions = new Veldrid.GraphicsDeviceOptions(false, null, syncToVBlank, ResourceBindingModel.Improved, true, true, _colorSrgb);
#if DEBUG
		gdOptions.Debug = true;
#endif
	...
	public void DestroyAllObjects()
	{
		...
		gr.obj.CommonMaterials.DestroyAllDeviceObjects();
		gr.StaticResourceCache.DestroyAllDeviceObjects();
		_gd.WaitForIdle();
	}
	...
	private void CreateAllObjects()
	{
		...
		gr.obj.CommonMaterials.CreateAllDeviceObjects( _gd, initCL, _sc );
